...
1. 出一个功能版。作为后续工作的 baseline，关键是 survey 要做好。
2. 大量实验调参、调数据，大面上的提升算法性能的精度。
3. 针对性的去 FA。比如用 post filter，加 loss 项等。
4. 模型加速，尽量不掉精度。
...
1. 标注错误的占比
2. 先看数据特征，加减数据重新训练。
3. 改超参数和网络结构，优化网络结构（暴力搜，NAS）
4. 改 loss 函数，增加 loss 项。思路参考多目标优化增加目标维度。
5. 上规则，加个 model，工程层面解决。
...
Loss 明显抖动，如何排查。跟找 hard sample 的思路差不多。

1. 用当前最好的 model，以 batch_size = 1 训练一个 epoch。得到每张图片的 loss
2. 分析 Loss 较大的图片特征。
3. 思考：跑一遍 inference，可能也可以。省掉了反向传播的过程。看需要额外写多少代码才能把 inference 跑起来。
