...
__global__ void reduce_naive(float* d_in, float* d_out) {
    ...
    unsigned int i = blockIdx.x * blockDim.x + tid;  // Global index
...
int main() {
    ...
    // Launch config
    const int block_size = 256;  // Typical starting value
    ...
    reduce_naive<<<grid_size, block_size, block_size*sizeof(float)>>>(d_in, d_out);
